...
"""
Single-file script to train a three-layer CNN on MNIST/Fashion-MNIST using SoftHebb and save the output features encoded as spikes with first-spike coding.
...
class DeepSoftHebb(nn.Module):
    def __init__(self, n_classes=10):
        ...
        self.classifier = nn.Linear(13824, n_classes)
        self.classifier.weight.data = 0.11048543456039805 * torch.rand(n_classes, 13824)
...
if __name__ == "__main__":
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = load_mnist_numpy(args.input_dir)
    ...
    trainset = ArrayMNIST(X_train, y_train)
    ...
    if X_val is not None:
        valset = ArrayMNIST(X_val, y_val)
        valloader = DataLoader(valset, batch_size=1000, shuffle=False)
        
    if X_test is not None:
        testset = ArrayMNIST(X_test, y_test)
