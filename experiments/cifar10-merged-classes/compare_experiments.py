"""
NCG vs Static Neuron Allocation: Comparison Visualization
==========================================================
Parses stdout.log files from multiple runs and generates:
  1. Train/Val accuracy learning curves
  2. Final active neurons per class bar chart
  3. Class difficulty trajectory
...
# Runs to compare: (label, log_path, color, linestyle)
RUNS = [
    (
        "NCG-Selective (dynamic)",
        os.path.join(SCRIPT_DIR, "output-csnn-ncg", "stdout.log"),
        "#2196F3",
        "-",
    ),
    (
        "Static-100 (20/class fixed)",
        os.path.join(SCRIPT_DIR, "output-csnn-static100", "stdout.log"),
        "#9C27B0",
        ":",
    ),
    (
        "Static-60 (12/class fixed)",
        os.path.join(SCRIPT_DIR, "output-csnn-static60", "stdout.log"),
        "#FF5722",
        "--",
    ),
    (
        "Static-200 (40/class fixed)",
        os.path.join(SCRIPT_DIR, "output-csnn-static200", "stdout.log"),
        "#4CAF50",
        "-.",
    ),
]
...
def parse_log(log_path):
    """Parse stdout.log, return list of epoch dicts."""
    ...
    # First pass: collect per-epoch data keyed by epoch number
    epoch_map = {}
    ...
    for i, line in enumerate(lines):
        ...
        if m2:
            # Attribute to the most recently opened epoch
            if epoch_map:
                ep = max(epoch_map)
                epoch_map[ep]["active_per_class"] = list(
                    map(int, m2.group(1).split(","))
                )
        ...
        mv = re.search(
            r"Accuracy on validation set after epoch (\d+): ([\d.]+)", line
        )
        if mv:
            ep = int(mv.group(1))
        ...
        mt = re.search(
            r"Accuracy on test set after epoch (\d+): ([\d.]+)", line
        )
        if mt:
            ep = int(mt.group(1))
...
def get_final_active(epochs, n_neurons_total=None):
    """Return final active neurons per class, or uniform allocation if no NCG data."""
    for ep in reversed(epochs):
        if "active_per_class" in ep:
            return ep["active_per_class"]
    # Static runs: all neurons active uniformly
...
def main():
    ...
    fig.suptitle(
        "NCG vs Static Neuron Allocation — Merged CIFAR10 (Balanced)",
        fontsize=15,
        fontweight="bold",
        y=0.98,
    )
    ...
    # Zoom y-axis: collect all finite accuracy values and pad ±3%
    all_accs = []
    ...
    # ---- 4. Final active neurons per class (grouped bar) ----
    ...
    # Shade complex / simple regions
    ax_active.axvspan(-0.5, 1.5, alpha=0.05, color="red",  label="_nolegend_")
    ax_active.axvspan(1.5,  2.5, alpha=0.05, color="yellow", label="_nolegend_")
    ax_active.axvspan(2.5,  4.5, alpha=0.05, color="green", label="_nolegend_")
    ...
    # ---- 5. Difficulty trajectory (NCG only) ----
    ...
    # -------------------------------------------------------------------------
    out_path = os.path.join(SCRIPT_DIR, "comparison.png")
