"""
CSNN-based Merged CIFAR10 Dataset Preparation
==============================================
Applies merge map to pre-extracted CSNN features
(ft-extract/extracted/STDP-CSNN/CIFAR10/kfold/0-run1/)
and balances each merged class to equal sample count.

Feature dim: 6272 (8x7x112 after conv+pool in this run)
...
CSNN_DIR = os.path.join(REPO_ROOT, "ft-extract/extracted/STDP-CSNN/CIFAR10/kfold/0-run1")
OUT_DIR  = os.path.join(os.path.dirname(__file__), "input-csnn")
...
def process_split(split_name, target):
    src = os.path.join(CSNN_DIR, f"{split_name}.npy")
    raw = np.load(src, allow_pickle=True).item()

    data   = raw["data"]       # list of sparse recarrays
    labels = raw["labels"]     # int16 array, 0-9
    shape  = raw["shape"]
...
if __name__ == "__main__":
    ...
    print("  python app/run.py experiments/cifar10-merged-classes/input-csnn \\")
    print("    experiments/cifar10-merged-classes/output-csnn-ncg \\")
